...
export const useAnalyticsCalculations = () => {
  const calculateUserMetrics = useCallback(
    (input: AnalyticsCalculationsInput) => {
      ...
      // Utilisateurs actifs (non verrouillés) créés jusqu'à la fin de la période
      ...
      const previousPeriodUsers = profiles.filter((p) => {
        ...
        const createdAt = new Date(p.created_at);
        return (
          createdAt >= previousPeriodStart && createdAt < previousPeriodEnd
        );
      }).length;
    ...
    (input: AnalyticsCalculationsInput, userMetrics: any) => {
      ...
      const averageOrderValue =
        currentPeriodPayments.length > 0
          ? totalRevenue / currentPeriodPayments.length
          : 0;
      ...
      const previousPeriodPayments = payments.filter((p) => {
        ...
        const paymentDate = new Date(p.created_at);
        return (
          paymentDate >= previousPeriodStart && paymentDate < previousPeriodEnd
        );
      });
    ...
    (input: AnalyticsCalculationsInput, userMetrics: any) => {
      ...
      const activeUsersGrowthRate =
        previousActiveUsers > 0
          ? ((userMetrics.activeUsers - previousActiveUsers) /
              previousActiveUsers) *
            100
      ...
      const previousCancellationRate =
        previousPeriodBookings > 0
          ? (previousCancelledBookings / previousPeriodBookings) * 100
          : 0;

      const cancellationRateChange =
        previousCancellationRate > 0
          ? currentCancellationRate - previousCancellationRate
          : 0;
