...
const ServiceDetailsTab: React.FC<{ service: ServiceWithDetails }> = ({ service }) => (
  <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
    ...
    <Card variant="outlined">
      <CardContent>
        ...
        {service.qualifications_required && service.qualifications_required.length > 0 && (
          <>
            <Divider sx={{ my: 2 }} />
            <Box>
              <Typography variant="subtitle2" color="text.secondary" gutterBottom>
...
const ProviderDetailsTab: React.FC<{ service: ServiceWithDetails }> = ({ service }) => {
  // Récupérer les vraies données du prestataire
  const { data: providerStats, isLoading: statsLoading } = useProviderStats(
    service.provider?.id || ""
  );
  
  const { data: otherServices, isLoading: servicesLoading } = useProviderServices(
    service.provider?.id || "", 
        ...
        <Card variant="outlined">
          <CardContent>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 3, mb: 3 }}>
              ...
              >
                {service.provider.first_name?.charAt(0) || service.provider.full_name?.charAt(0) || "P"}
              </Avatar>
              <Box sx={{ flex: 1 }}>
            ...
            <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
              ...
              {service.provider.phone && (
                <Box sx={{ flex: 1, minWidth: 200 }}>
                  ...
                  <Typography variant="body1">{service.provider.phone}</Typography>
                </Box>
              )}
            </Box>
          </CardContent>
              ...
              <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                <Box sx={{ flex: 1, minWidth: 120 }}>
                  <Box sx={{ textAlign: "center", p: 1, bgcolor: "primary.light", borderRadius: 1 }}>
                    <Typography variant="h4" color="primary.main">{providerStats.activeServices}</Typography>
                    <Typography variant="caption">Services actifs</Typography>
                  </Box>
                </Box>
                <Box sx={{ flex: 1, minWidth: 120 }}>
                  <Box sx={{ textAlign: "center", p: 1, bgcolor: "success.light", borderRadius: 1 }}>
                    <Typography variant="h4" color="success.main">{providerStats.completedRequests}</Typography>
                ...
                <Box sx={{ flex: 1, minWidth: 120 }}>
                  <Box sx={{ textAlign: "center", p: 1, bgcolor: "warning.light", borderRadius: 1 }}>
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Typography variant="h4" color="warning.main">{providerStats.averageRating.toFixed(1)}</Typography>
            ...
            {servicesLoading ? (
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {otherServices.map((otherService) => (
                  <Chip 
                    key={otherService.id}
                    label={`${otherService.name} (${formatCurrency(otherService.base_price)})`}
...
const ServiceHistoryTab: React.FC<{ service: ServiceWithDetails }> = ({ service }) => {
  const { data: history, isLoading: historyLoading } = useServiceHistory(service.id);
  const { data: performance, isLoading: performanceLoading } = useServicePerformance(service.id);
              ...
              <Box key={request.id} sx={{ 
                ...
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Client: {request.requester?.full_name || request.requester?.email || "Inconnu"} • {" "}
          ...
          <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
            <Box sx={{ flex: 1, minWidth: 200 }}>
              <Box sx={{ p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                <Typography variant="h4" color="primary">{performance.thisMonthRequests}</Typography>
                <Typography variant="body2" color="text.secondary">Demandes ce mois</Typography>
              </Box>
            </Box>
            <Box sx={{ flex: 1, minWidth: 200 }}>
              <Box sx={{ p: 2, bgcolor: 'background.default', borderRadius: 1 }}>
                <Typography variant="h4" color="success.main">{performance.satisfactionRate}%</Typography>
                <Typography variant="body2" color="text.secondary">Taux de satisfaction</Typography>
              </Box>
            </Box>
          </Box>
        ) : (
...
export const ServiceInfoSections: React.FC<ServiceInfoSectionsProps> = ({
  ...
  return (
    <Box>
      ...
      {activeTab === 1 && (
        <ProviderDetailsTab service={service} />
      )}
      
      {activeTab === 2 && (
        <ServiceHistoryTab service={service} />
      )}
    </Box>
  );
};