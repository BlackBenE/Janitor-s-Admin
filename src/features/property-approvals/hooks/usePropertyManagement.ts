...
export const usePropertyManagement = () => {
  ...
  // Notifications
  const [notification, setNotification] = useState<PropertyNotificationState>(initialNotification);
  ...
  const exportPropertiesToCSV = (properties: any[]) => {
    try {
      ...
      showNotification(`Successfully exported ${properties.length} properties to CSV`, 'success');
    } catch (error) {
