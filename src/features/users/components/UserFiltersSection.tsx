...
import { userFilterConfigs } from '@/shared/config/filterConfigs';
import { GenericTabs } from '@/shared/components/navigation';
import { userTabConfigs, getUserCount } from '@/shared/config/tabConfigs';
import { UserFilters, UserProfile, UserRole } from '@/types/userManagement';
    ...
    <Box>
      ...
      {selectedUsers.length > 0 && (
        <Box sx={{ display: 'flex', gap: 1, ml: 'auto', flexWrap: 'wrap' }}>
          <Button variant="outlined" color="success" size="small" onClick={onBulkValidate}>
            Validate Selected ({selectedUsers.length})
          </Button>

          <Button variant="outlined" color="warning" size="small" onClick={onBulkSetPending}>
            Set Pending ({selectedUsers.length})
          </Button>

          <Button variant="outlined" color="error" size="small" onClick={onBulkSuspend}>
            Suspend Selected ({selectedUsers.length})
          </Button>

          <Button variant="outlined" color="info" size="small" onClick={onBulkUnsuspend}>
