...
export const USER_QUERY_KEYS = {
  ...
  list: (filters?: Partial<UserProfile>) => [...USER_QUERY_KEYS.lists(), filters] as const,
  ...
  activity: (userId: string) => [...USER_QUERY_KEYS.activities(), userId] as const,
  bookings: (userId: string) => [...USER_QUERY_KEYS.all, 'bookings', userId] as const,
  audit: (userId: string) => [...USER_QUERY_KEYS.all, 'audit', userId] as const,
  clientServices: (userId: string) => [...USER_QUERY_KEYS.all, 'client-services', userId] as const,
  ...
  interventions: (userId: string) => [...USER_QUERY_KEYS.all, 'interventions', userId] as const,
  search: (term: string) => [...USER_QUERY_KEYS.lists(), 'search', term] as const,
} as const;
...
export const useUserStats = (options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [totalUsersResult, activeUsersResult, vipUsersResult, lockedUsersResult] =
        await Promise.all([
 ...
 */
export const useUserStatsIndividual = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (userProfile.role === 'property_owner') {
        ...
        return {
          ...
          totalSpent: paymentsReceived.reduce((sum, p) => sum + (p.amount || 0), 0), // Total reçu
          ...
          averageBookingValue:
            bookings.length > 0
              ? paymentsReceived.reduce((sum, p) => sum + (p.amount || 0), 0) / bookings.length
              : 0,
        };
      } else {
        ...
        const correctTotal = paymentsForBookings.reduce((sum, p) => sum + (p.amount || 0), 0);
        ...
        return {
          ...
          cancelledBookings: bookings.filter((b) => b.status === 'cancelled').length,
          lastBookingDate: bookings[0]?.created_at || null,
          averageBookingValue: bookings.length > 0 ? correctTotal / bookings.length : 0, // ✅ Moyenne corrigée
        };
      }
    },
    enabled: options?.enabled !== false && !!userId,
 ...
 */
export const useUserActivity = (userIds: string[], options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      userIds.forEach((userId) => {
        ...
        const userServiceRequests = serviceRequests.filter((sr) => sr.client_id === userId);
        const userInterventions = interventions.filter((i) => i.provider_id === userId);
        const userSubscriptions = subscriptions.filter((s) => s.user_id === userId);

        // Calculs de base (pour tous les rôles)
        let totalSpent = userPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
        let totalEarned = 0;
        ...
        switch (userRole) {
          ...
          case 'property_owner':
            ...
            const ownerBookings = bookings.filter((booking) =>
              userProperties.some((property) => property.id === booking.property_id)
            );
            totalEarned = ownerBookings.reduce((sum, b) => sum + (b.total_amount || 0), 0);
            break;

          case 'service_provider':
          case 'provider':
            // Les prestataires gagnent via leurs interventions
            totalEarned = userInterventions.reduce((sum, i) => sum + (i.amount || 0), 0);
            break;
 ...
 */
export const useUserSearch = (searchTerm: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserBookings = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        throw new Error(`Erreur lors de la récupération des bookings: ${error.message}`);
      }
 ...
 */
export const useUserAuditLog = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserClientServices = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserProviderServices = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserServiceRequests = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.warn('Provider service requests not accessible, returning empty array');
        return [];
      }
    },
    enabled: options?.enabled !== false && !!userId,
 ...
 */
export const useUserInterventions = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserSubscriptions = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
 ...
 */
export const useUserServicesStats = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [serviceRequestsResponse, paymentsResponse] = await Promise.all([
        supabase.from('service_requests').select('*').eq('requester_id', userId),
        supabase.from('payments').select('*').eq('payer_id', userId).eq('payment_type', 'service'), // 🎯 Seulement les services
      ]);
      ...
      return {
        ...
        totalSpentOnServices: payments.reduce((sum, p) => sum + (p.amount || 0), 0),
        completedServices: serviceRequests.filter((s) => s.status === 'completed').length,
        pendingServices: serviceRequests.filter((s) => s.status === 'pending').length,
        averageServiceValue:
          serviceRequests.length > 0
            ? payments.reduce((sum, p) => sum + (p.amount || 0), 0) / serviceRequests.length
            : 0,
      };
    },
    enabled: options?.enabled !== false && !!userId,
 ...
 * Pour PaymentsPage
 */
export const useUserFinancialStats = (userId?: string, options?: { enabled?: boolean }) => {
  return useQuery({
    queryKey: [...USER_QUERY_KEYS.all, 'financial-stats', userId],
    queryFn: async () => {
      if (!userId) return null;

      const { data: payments } = await supabase.from('payments').select('*').eq('payer_id', userId);

      const paymentsData = payments || [];

      // Grouper par type de paiement
      const groupedByType = paymentsData.reduce(
        (acc, payment) => {
