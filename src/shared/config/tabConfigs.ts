import { TabConfig } from '@/shared/components/navigation/GenericTabs';
...
export const userTabConfigs: TabConfig<UserRole | null>[] = USER_TABS.map((tab) => ({
  key: tab.role,
  label: tab.label,
  icon: tab.icon,
...
export const propertyTabConfigs: TabConfig<PropertyStatus>[] = PROPERTY_TABS.map((tab) => ({
  key: tab.status,
  label: tab.label,
  icon: tab.icon,
  color: 'default' as const,
...
export const getUserCount = (tabKey: UserRole | null, users: UserProfile[]): number => {
  if (tabKey === null) {
...
export const getPropertyCount = (tabKey: PropertyStatus, properties: any[]): number => {
  ...
  return properties.filter((property) => property.validation_status === statusMapping[tabKey])
    .length;
};
